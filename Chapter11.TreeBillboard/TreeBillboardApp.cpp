...
bool TreeBillboardApp::Init()
{
	...
	ID3D11Resource* grassMapResource = nullptr;
	ID3D11Resource* waveMapResource = nullptr;
	ID3D11Resource* wireFenceResource = nullptr;
	ID3D11Resource* treeArrayResource = nullptr;
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/grass.dds", &grassMapResource, &mGrassMapSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/water2.dds", &waveMapResource, &mWavesMapSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/WireFence.dds", &wireFenceResource, &mBoxMapSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/treearray.dds", &treeArrayResource, &mTreeTextureMapArraySRV));
	
	ReleaseCOM(grassMapResource); // view saves reference
	ReleaseCOM(waveMapResource); // view saves reference
	ReleaseCOM(wireFenceResource); // view saves reference
	ReleaseCOM(treeArrayResource); // view saves reference
