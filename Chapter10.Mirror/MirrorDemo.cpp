...
bool MirrorApp::Init()
{
	...
	ID3D11Resource* checkboardResource = nullptr;
	ID3D11Resource* brickResource = nullptr;
	ID3D11Resource* iceResource = nullptr;
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/checkboard.dds", &checkboardResource, &mFloorDiffuseMapSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/brick01.dds", &brickResource, &mWallDiffuseMapSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/ice.dds", &iceResource, &mMirrorDiffuseMapSRV));

	ReleaseCOM(checkboardResource); // view saves reference
	ReleaseCOM(brickResource); // view saves reference
	ReleaseCOM(iceResource); // view saves reference
