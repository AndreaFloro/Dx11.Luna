...
bool CameraApp::Init()
{
	...
	ID3D11Resource* floorMapResource = nullptr;
	ID3D11Resource* stoneMapResource = nullptr;
	ID3D11Resource* bricksFenceResource = nullptr;
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/floor.dds", &floorMapResource, &mFloorTexSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/stone.dds", &stoneMapResource, &mStoneTexSRV));
	HR(DirectX::CreateDDSTextureFromFile(md3dDevice, L"Textures/bricks.dds", &bricksFenceResource, &mBrickTexSRV));

	ReleaseCOM(floorMapResource); // view saves reference
	ReleaseCOM(stoneMapResource); // view saves reference
	ReleaseCOM(bricksFenceResource); // view saves reference
